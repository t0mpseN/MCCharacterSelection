...
public class CharacterListScreen extends Screen {
    ...
    private ItemStack drawRightPanel(DrawContext ctx, TextRenderer tr, CharacterDto c, int centerPanelRightX, int centerPanelY, int centerHeight, int mouseX, int mouseY) {
        ...
        Text stat1 = Text.literal("Blocks: ").setStyle(net.minecraft.text.Style.EMPTY.withFont(CUSTOM_FONT)).formatted(Formatting.GRAY).append(Text.literal(stats.blocksMined() > 0 ? String.valueOf(stats.blocksMined()) : "---").formatted(Formatting.WHITE));
        Text stat2 = Text.literal("Time: ").setStyle(net.minecraft.text.Style.EMPTY.withFont(CUSTOM_FONT)).formatted(Formatting.GRAY).append(Text.literal(timeStr).formatted(Formatting.WHITE));
        Text stat3 = Text.literal("Kills: ").setStyle(net.minecraft.text.Style.EMPTY.withFont(CUSTOM_FONT)).formatted(Formatting.GRAY).append(Text.literal(stats.mobKills() > 0 ? String.valueOf(stats.mobKills()) : "---").formatted(Formatting.WHITE));
        Text stat4 = Text.literal("Diamonds: ").setStyle(net.minecraft.text.Style.EMPTY.withFont(CUSTOM_FONT)).formatted(Formatting.GRAY).append(Text.literal(stats.diamonds() > 0 ? String.valueOf(stats.diamonds()) : "---").formatted(Formatting.WHITE));

        int col1 = px + 20;
        int col2 = px + pw / 2 + 6;

        drawRetroText(ctx, tr, stat1, col1, extraY, 0xFFFFFF);
        drawRetroText(ctx, tr, stat2, col2, extraY, 0xFFFFFF);
        ...
        if (latestAdv != null) {
            ...
            Text titleTxt = Text.literal(latestAdv.title()).setStyle(net.minecraft.text.Style.EMPTY.withFont(CUSTOM_FONT)).formatted(Formatting.WHITE);
            drawRetroText(ctx, tr, titleTxt, badgeX + 28, badgeY + 8, 0xFFFFFF);

            Text descText = Text.literal(latestAdv.description()).setStyle(net.minecraft.text.Style.EMPTY.withFont(CUSTOM_FONT));
            ...
            ctx.fill(badgeX, badgeY, badgeX + badgeW, badgeY + badgeH, 0xFF1A1A1A);
            ctx.drawBorder(badgeX, badgeY, badgeW, badgeH, 0xFF555555);
            Text noAdv = Text.literal("No advancements yet").setStyle(net.minecraft.text.Style.EMPTY.withFont(CUSTOM_FONT)).formatted(Formatting.DARK_GRAY);
            drawRetroText(ctx, tr, noAdv, badgeX + (badgeW - tr.getWidth(noAdv)) / 2, badgeY + (badgeH / 2) - 4, 0xFFFFFF);
        }
